...
// Сетка карточек статей: 1 колонка на самых узких, до 4 на широких мониторах
...
export function KbView() {
  ...
  // На мобильном сайдбар категорий — выдвижная панель поверх контента (как мобильное меню
  // навигации), а не фиксированная колонка: та съедала бы половину экрана 320-375px.
  ...
  const [view, setView] = usePersistentState<'list' | 'grid'>('view-mode-kb', 'list')
  const [filtersOpen, setFiltersOpen] = usePersistentState('filters-open-kb', true)
  ...
  // Общее содержимое панели категорий — используется и в десктопной колонке, и в мобильном
  // drawer. onSelect параметризован, т.к. на мобильном выбор категории ещё и закрывает drawer.
  const categoryPanelBody = (onSelect: (id: number | null) => void) => (
    <>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700/60 shrink-0">
        <div className="flex flex-wrap items-end justify-between gap-x-2 gap-y-3 mb-4">
          <div className="min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-slate-100">База знаний</h1>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              title="Категории"
              className="md:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
            >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* Мобильный drawer с категориями — отдельный оверлей поверх контента, не часть flex-раскладки */}
        <div
          className={cn(
            'md:hidden fixed inset-0 bg-black/50 z-40 transition-opacity duration-200',
            mobileCatOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          )}
          onClick={() => setMobileCatOpen(false)}
        />
        <div className={cn(
          'md:hidden fixed top-0 left-0 h-full w-72 max-w-[85vw] bg-white dark:bg-slate-900 z-50 flex flex-col shadow-2xl',
          'transition-transform duration-200 ease-in-out',
          mobileCatOpen ? 'translate-x-0' : '-translate-x-full'
        )}>
          <div className="flex items-center justify-between px-3 py-3 border-b border-slate-100 dark:border-slate-700/60 shrink-0">
            <span className="font-semibold text-sm text-slate-800 dark:text-slate-100">Категории</span>
            <button onClick={() => setMobileCatOpen(false)} className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
              <ChevronLeftIcon className="w-4 h-4" />
            </button>
          </div>
          {categoryPanelBody(id => { handleCatSelect(id); setMobileCatOpen(false) })}
        </div>

        {panelWidth > 0 && (
        <div
          className={cn('hidden md:flex shrink-0 border-r border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-900 flex-col overflow-hidden', isSnapping && 'transition-[width] duration-150')}
          style={isDesktop ? { width: panelWidth } : { width: CAT_DEFAULT }}
        >
          {categoryPanelBody(handleCatSelect)}
        </div>
        )}
        ...
        {panelWidth === 0 ? (
          <button
            ...
            className="hidden md:flex shrink-0 w-5 items-center justify-center bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-700/60 text-slate-300 hover:text-[#1B3A72] dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          >
          ...
          <div
            ...
            className="hidden md:flex shrink-0 w-1 cursor-col-resize bg-slate-200 dark:bg-slate-700 hover:bg-[#4A8FE7]/60 transition-colors duration-100 group items-center justify-center"
...
function AttachmentRow({ att, articleId, onDelete, deleting }: {
  ...
  return (
    <div
      ...
      className="flex items-center gap-3 px-4 sm:px-6 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 group cursor-pointer"
    >
