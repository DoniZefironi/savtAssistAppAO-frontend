...
import { kbApi, type Tag } from '@/lib/api/kb'
import { useFolderUpload } from '@/lib/hooks/use-folder-upload'
import { fmtSize, validateDocFile, DOC_ACCEPT } from '@/components/cabinets/cabinet-dialog-shared'
import { FileIcon, PdfIcon, ImageIcon, TrashIcon, UploadIcon, DownloadIcon, TagIcon, PencilIcon } from '@/components/cabinets/cabinet-dialog-icons'

// Документация проекта в целом (не привязанная к конкретному ШУ) — тот же UX,
// что и DocsTab для ШУ (см. cabinet-docs-tab.tsx), но через project_id вместо
// cabinet_id, см. README-backend.md, «Рут admin: documents».
export function ProjectDocsTab({ projectId, isAdmin }: { projectId: number; isAdmin: boolean }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => mediaApi.listProjectDocuments(projectId),
  })
  ...
  const folderUpload = useFolderUpload({
    ...
    upload: (file) => mediaApi.uploadProjectDocument(projectId, file, file.name.replace(/\.[^.]+$/, ''), false),
    ...
    <div onDragOver={e => e.preventDefault()} onDrop={handleDrop}>
      ...
      {!isLoading && docs.length === 0 && (
        <div className="flex flex-col items-center justify-center py-10 text-slate-400">
          ...
          <p className="text-sm">Нет документов проекта</p>
        </div>
      )}
...
function DocRow({ doc, projectId, allTags, isAdmin, onOpen, onDownload, onDelete, deleting }: {
  doc: CabinetDocument
  projectId: number
  allTags: Tag[]
  isAdmin: boolean
  ...
  return (
    <div className="group">
      <div onClick={onOpen} className="flex items-center gap-3 px-1 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer">
      ...
      {editingAccess && (
        <div onClick={e => e.stopPropagation()} className="px-1 pb-3 pt-2 space-y-1.5 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-700/30">
          <label className="flex items-center gap-2 text-xs text-slate-500 cursor-pointer select-none">
            <input
              type="checkbox"
      ...
      {editingTags && (
        <div onClick={e => e.stopPropagation()} className="px-1 pb-3 space-y-2 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-700/30">
          ...
          <div className="relative">
            ...
            {tagInput && (filtered.length > 0 || showCreate) && (
              <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg shadow-lg overflow-hidden max-h-36 overflow-y-auto animate-in fade-in-0 zoom-in-95 duration-100">
                {filtered.map(tag => (
