...
- [ ] **Предусмотреть выход из тупика на экране ввода кода.** Если этот Telegram уже привязан к аккаунту, бот откажет **в Telegram**, а приложение будет молча ждать код; `resend` при этом снова пошлёт в Telegram — получится круг. Нужна подсказка «Код не приходит? Посмотрите ответ бота в Telegram» и кнопка «Войти». Подробности — в разделе «Один Telegram — один аккаунт».
...
> ⚠️ **Известная дыра в UX, учтите при вёрстке.** Объяснение приходит в Telegram, а
> приложение остаётся на экране ввода кода и ждёт код, которого не будет.
> `/register/resend` в этом состоянии вернёт непустой `deep_link` (номер так и не
> подтверждён), то есть по инструкции клиент отправит пользователя обратно в
> Telegram — и получится замкнутый круг, пока через 25 минут не истечёт
> `registration_token` и `/register/complete` не ответит `404`.
>
> Пока на сервере нет отдельного статуса регистрации, на экране ввода кода нужна
> подсказка «Код не приходит? Посмотрите ответ бота в Telegram» и кнопка «Войти» —
> чтобы человек мог выйти из тупика сам.
