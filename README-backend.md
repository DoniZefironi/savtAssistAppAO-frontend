...
- ШУ ничей (`project_id IS NULL`) → привязывается к проекту заявки, все
  участники проекта сразу получают доступ (чаты заводятся сразу же, без
  ожидания первого открытия);
...
```
`warranty_status`: `active`, `expiring_soon` (≤30 дней), `expired`. Список ШУ —
это все шкафы проектов, в которых состоит пользователь (доступ выводится из
проекта, не хранится по-шкафно) — `project_id`/`project_name` показывают, через
какой именно проект открыт доступ к каждому.
...
Убрать пользователя из проекта — теряет доступ разом ко всем его шкафам. Только
для админа. Логируется в `audit_log`.
```json
