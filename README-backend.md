...
**Сортировка** — `sort_by`: `name`, `created_at` (по умолч.), `production_number`, `year`, `company_name`, `shipment_planned_at`, `shipment_actual_at`, `warranty_ends_at`, `cabinet_count`. `sort_order` — `asc` / `desc`. Пустые значения всегда уходят в конец в обе стороны: проект без даты отгрузки не занимает верх списка, отсортированного по ней. При равных значениях порядок стабильный (по `id`), поэтому строки не «прыгают» между страницами.
